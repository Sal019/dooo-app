...
import com.dooo.android.utils.ytExtractor.LiveVideoInfoCallback;
...
public class Player extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView contentFirstName = playerView.findViewById(R.id.contentFirstName);
        TextView contentSecondName = playerView.findViewById(R.id.contentSecondName);
        ...
        ImageView Live_logo = playerView.findViewById(R.id.Live_logo);
        if (content_type == 3) {
        ...
        ImageView img_full_scr = playerView.findViewById(R.id.img_full_scr);
        TextView img_full_scr_txt = playerView.findViewById(R.id.img_full_scr_txt);
        img_full_scr.setOnClickListener(view -> {
            if (playerView.getResizeMode() == AspectRatioFrameLayout.RESIZE_MODE_FILL) {
                playerView.setResizeMode(AspectRatioFrameLayout.RESIZE_MODE_ZOOM);
                img_full_scr.setImageDrawable(getDrawable(R.drawable.expand));
                img_full_scr_txt.setText("FullScreen");
                playerView.getSubtitleView().setBottomPaddingFraction(0.18f);
            } else if (playerView.getResizeMode() == AspectRatioFrameLayout.RESIZE_MODE_ZOOM) {
                playerView.setResizeMode(AspectRatioFrameLayout.RESIZE_MODE_FIT);
                img_full_scr.setImageDrawable(getDrawable(R.drawable.compress));
                img_full_scr_txt.setText("BestFit");
                ...
                img_full_scr.setImageDrawable(getDrawable(R.drawable.expand_wide));
                img_full_scr_txt.setText("FitScreen");
                playerView.getSubtitleView().setBottomPaddingFraction(0.1f);
            } else {
                playerView.setResizeMode(AspectRatioFrameLayout.RESIZE_MODE_FIT);
                img_full_scr.setImageDrawable(getDrawable(R.drawable.compress));
        ...
        ImageView img_settings = playerView.findViewById(R.id.img_settings);
        ...
        ImageView Back_btn_img = playerView.findViewById(R.id.Back_btn_img);
        ...
        Skip_Intro_btn = playerView.findViewById(R.id.Skip_Intro_btn);
        ...
        Play_Next_btn = playerView.findViewById(R.id.Play_Next_btn);
        ...
        ImageView imgBrightness = playerView.findViewById(R.id.img_Brightness);
        BoxedVertical brightness = playerView.findViewById(R.id.brightness);
        ConstraintLayout brightnessLayout = playerView.findViewById(R.id.brightnessLayout);
        ...
        if(tinyDB.getInt("player_brightness") == 0) {
            brightness.setValue(getBrightness(this));
            setBrightness(context, getBrightness(this));
            tinyDB.putInt("player_brightness", getBrightness(this));
        } else {
            brightness.setValue(tinyDB.getInt("player_brightness"));
            setBrightness(context, tinyDB.getInt("player_brightness"));
        }
        ...
        brightness.setOnBoxedPointsChangeListener(new BoxedVertical.OnValuesChangeListener() {
            ...
            public void onPointsChanged(BoxedVertical boxedPoints, int points) {
                ...
                tinyDB.putInt("player_brightness", boxedPoints.getValue());
            }
        ...
        ImageView imgVolume = playerView.findViewById(R.id.img_Volume);
        BoxedVertical volume = playerView.findViewById(R.id.volume);
        ConstraintLayout volumeLayout = playerView.findViewById(R.id.volumeLayout);
    ...
    private Runnable runnable = new Runnable() {
        ...
        public void run() {
            if(simpleExoPlayer != null) {
                if(introVideoSource!=null) {
                    if(simpleExoPlayer.getCurrentMediaItem() != introVideoSource.getMediaItem()) {
    ...
    void Prepare_Source(String source, String url, String DrmUuid, String DrmLicenseUri) {
        ...
        if (source.equalsIgnoreCase("m3u8")) {
            ...
            progressDialog.show();
            YouTubeExtractor.getVideoInfo(url, new VideoInfoCallback() {
                @Override
                public void onVideoUrlReceived(final List<org.schabi.newpipe.extractor.stream.VideoStream> videoStreams) {
                    runOnUiThread(() -> {
                        ...
                        if(videoStreams.isEmpty()) {
                            ...
                            multiQualityPlayer("mp4", videoStreams.get(0).getUrl(), "", "");
                            progressDialog.hide();
                        } else {
                            ...
                            AlertDialog.Builder builder = new AlertDialog.Builder(context)
                                    ...
                                    .setItems(name, (dialog, which) -> {
                                        multiQualityPlayer("mp4", videoStreams.get(which).getUrl(), "", "");
                                    })
            ...
            progressDialog.show();
            YouTubeExtractor.getLiveVideoInfo(url, new LiveVideoInfoCallback() {
                @Override
                public void onVideoUrlReceived(final String videoStream) {
                    runOnUiThread(() -> {
                        MediaItem mediaItem = new MediaItem.Builder()
                                .setMimeType(MimeTypes.APPLICATION_M3U8)
                                .setUri(videoStream)
                ...
                || source.equals("Facebook") || source.equals("GoogleDrive") || source.equals("MediaShore")
                ...
                || source.equals("Voot") || source.equals("Vudeo") || source.equals("Vimeo")
                || source.equals("Yandex") || source.equals("vidtube")) {
            progressDialog.show();

            new DoooStream(context, bGljZW5zZV9jb2Rl, AppConfig.rawUrl, AppConfig.apiKey, new DoooStream.OnInitialize() {
                @Override
                public void onSuccess(DoooStream doooStream) {
                    doooStream.find(source, url, false, new DoooStream.OnTaskCompleted() {
                        ...
                        public void onSuccess(List<StreamList> streamList) {
                            Log.d("test", streamList.toString());
                            multiQualityDialog(streamList);
                        }
        ...
        } else if (source.equals("Streamwish") || source.equals("GogoAnime")) {
            progressDialog.show();
            ...
            webView.loadUrl(url);
        } else if (source.equals("Vidhide") ) {
            progressDialog.show();
            getPublicIpAddress(context, new IpCallback() {
                @Override
    ...
    MediaSource getIntroVideoSource(MediaSource mediaSource) {
        if(resumePosition == 0) {
            try {
                ...
                JsonObject player_intro_Object = new Gson().fromJson(sharedPreferences.getString("Config", null), JsonObject.class);
                if(!player_intro_Object.get("player_intro").isJsonNull())
                {
                    if(!player_intro_Object.get("player_intro").getAsString().isEmpty()) {
                        String player_intro = player_intro_Object.get("player_intro").getAsString();
                        DataSource.Factory httpDataSourceFactory = new DefaultHttpDataSource.Factory()
                                .setUserAgent(userAgent)
                                .setKeepPostFor302Redirects(true)
                                .setAllowCrossProtocolRedirects(true)
                        ...
                        introVideoSource = new ProgressiveMediaSource.Factory(dataSourceFactory)
                                .createMediaSource(MediaItem.fromUri(Uri.parse(player_intro)));
                        playerView.hideController();
                        return new ConcatenatingMediaSource(introVideoSource, mediaSource);
                    }
                }
            } catch (Exception e) {}
        }
    ...
    void initializePlayer(MediaSource mediaSource1) {
        MediaSource mediaSource = getIntroVideoSource(mediaSource1);
...
//                ,sharedPreferences.getBoolean(SOFTWARE_EXTENTIONS, false));
    ...
    private void loadConfig() {
        ...
        bGljZW5zZV9jb2Rl = jsonObject.get("license_access_token").getAsString();
    }
    ...
    public static void getPublicIpAddress(Context context, final IpCallback callback) {
        RequestQueue queue = Volley.newRequestQueue(context);
