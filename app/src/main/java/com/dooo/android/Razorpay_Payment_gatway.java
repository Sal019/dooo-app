...
public class Razorpay_Payment_gatway extends BaseActivity implements PaymentResultListener {
    ...
    public void onPaymentSuccess(String razorpayPaymentID) {
        ...
        StringRequest sr = new StringRequest(Request.Method.POST, AppConfig.url + "dXBncmFkZQ", response -> {
            if(response.equals("Account Upgraded Successfully")) {
                ...
                handler.postDelayed(() -> {
                    finishAffinity();
                    startActivity(new Intent(Razorpay_Payment_gatway.this, Splash.class));
                }, 2000);
            } else {
                finish();
            }
    ...
    void loadSubscriptionDetails(int id) {
        ...
        StringRequest sr = new StringRequest(Request.Method.GET, AppConfig.url +"getSubscriptionDetails/"+id, response -> {
            if(!response.equals("No Data Avaliable")) {
                ...
                strCurrency = HelperUtils.getCurrencyName(currency);
